...
export default function ComparableCompanyAnalysis({ data }: { data: CompanyData }) {
  ...
  return <section className="analysis-section comps-analysis" id="competitors">
    ...
    <div className="peer-table-wrap table-scroll"><table className="peer-table"><thead><tr><th>Company</th><th>Business focus</th><th>Market cap</th><th>Revenue growth</th><th>Operating margin</th><th>EV / Revenue</th><th>EV / EBITDA</th><th>P / E</th></tr></thead><tbody>
      ...
      {peers.length > 0 && <tr className="peer-median"><td><b>PEER MEDIAN</b><span>{peers.length} returned companies</span></td><td>N/A</td><td>{formatCap(metrics.marketCap)}</td><td>{formatMetric(metrics.growth, "%")}</td><td>{formatMetric(metrics.margin, "%")}</td><td>{financialCompany ? "n/m" : formatMetric(metrics.revenueMultiple)}</td><td>{financialCompany ? "n/m" : formatMetric(metrics.multiple)}</td><td>{formatMetric(metrics.pe)}</td></tr>}
      {peers.length > 0 && <tr className="peer-mean"><td><b>PEER MEAN</b><span>Arithmetic average</span></td><td>N/A</td><td>{formatCap(means.marketCap)}</td><td>{formatMetric(means.growth, "%")}</td><td>{formatMetric(means.margin, "%")}</td><td>{financialCompany ? "n/m" : formatMetric(means.revenueMultiple)}</td><td>{financialCompany ? "n/m" : formatMetric(means.multiple)}</td><td>{formatMetric(means.pe)}</td></tr>}
    </tbody></table></div>
    {!peers.length && <div className="peer-empty">Comparable ratios were not returned for this request.</div>}
    <details className="peer-details"><summary>Business comparison</summary><div className="business-review"><article className="business-difference-card"><h3>{detailed.title}</h3><p>{detailed.summary}</p><div className="business-difference-grid">{detailed.dimensions.map((dimension) => <div key={dimension.label}><b>{dimension.label}</b><p>{dimension.detail}</p></div>)}</div>{detailed.peerModels.length > 0 && <div className="peer-model-list"><b>Peers</b><div>{detailed.peerModels.map((peer) => <section key={peer.symbol}><strong>{peer.symbol}: {peer.name}</strong><p>{peer.detail}</p></section>)}</div></div>}</article></div><h3 className="difference-title">Numbers</h3><div className="difference-grid">{insights.map((insight) => <article key={insight.label}><span>{insight.label}</span><strong>{insight.value}</strong><p>{insight.detail}</p></article>)}</div>{Boolean(comparison?.operatingCompetitors?.length) && <p className="peer-disclaimer">Other competitors: {comparison?.operatingCompetitors?.join(", ")}</p>}</details>
    <details className="peer-methodology"><summary>Data notes</summary><p>{comparison?.selectionBasis || "Peers are selected using products, customers, and operating model."}</p><p>Mean is the arithmetic average; median is less sensitive to outliers. Multiples use current market capitalization and latest annual financials, not forward consensus.</p></details>
  </section>;
}
