...
export default function CompanyAnalysisPage() {
  ...
  return <main className="analysis-page">
    ...
    <section className="analysis-section default-section">
      ...
      <div className="analysis-metrics five"><Metric label="Debt / revenue" value={ratio(defaultRisk.ratios.debtToRevenue)} detail="1.0× means debt equals one full year of revenue. Lower usually gives the company more financial flexibility, although profit margins still matter."/><Metric label="Net debt / EBITDA" value={ratio(defaultRisk.ratios.netDebtToEbitda)} detail="Roughly how many years of EBITDA would cover debt after subtracting cash. Negative means cash exceeds debt; above 4× is stressed in this screen."/><Metric label="Current ratio" value={ratio(defaultRisk.ratios.currentRatio)} detail="$1.50 means $1.50 of near-term assets for every $1 of near-term liabilities. Below 1.0× is a liquidity warning."/><Metric label="Interest coverage" value={ratio(defaultRisk.ratios.interestCoverage)} detail="How many times operating profit covers annual interest. Below 1.0× means operations are not currently covering interest expense."/><Metric label="FCF / debt" value={percentageRatio(defaultRisk.ratios.fcfToDebt)} detail="The share of funded debt one year of free cash flow could repay. 20% is about one-fifth; a negative number means cash burn."/></div>
