...
export default function ComparableCompanyAnalysis({ data }: { data: CompanyData }) {
  ...
  const qualityKey = (key: (typeof compKeys)[number]) => `${key}Quality` as keyof ComparableCompany;
  const usablePeerValues = (key: (typeof compKeys)[number]) => peers.map((peer) => {
    const quality = peer[qualityKey(key)];
    return quality === "extreme" || quality === "not-meaningful" || quality === "unavailable" ? null : peer[key] ?? null;
  });
  const compMedian = Object.fromEntries(compKeys.map((key) => [key, median(usablePeerValues(key))])) as Record<(typeof compKeys)[number], number | null>;
  const compMean = Object.fromEntries(compKeys.map((key) => [key, mean(usablePeerValues(key))])) as Record<(typeof compKeys)[number], number | null>;
